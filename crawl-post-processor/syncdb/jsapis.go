...
func getSyncJSAPIUsageIter(db *mgo.Database) (*mgo.Iter, error) {
	sourceMatch := bson.M{}

	// optionally add date-range filtering on the timestamp in _id
	...
	sourceProject, err := BuildProjection(reflect.TypeOf(syncJSAPIUsageInputRecord{}))
	if err != nil {
		return nil, err
	}
...
func syncJSAPIUsage(db *mgo.Database, sqlDb *sql.DB) error {
	log.Println("syncJSAPIUsage: getting per-page JS API usage iterator...")
	iter, err := getSyncJSAPIUsageIter(db)
	if err != nil {
		return err
	}
	defer func() {
	...
	err = CreateImportTable(sqlDb, "js_api_usage_import_schema", "import_js_api_usage")
	if err != nil {
	...
	importRows, err := BulkInsertRows(sqlDb, "syncJSAPIUsage", "import_js_api_usage", jsAPIUsageImportFields[:], func() ([]interface{}, error) {
